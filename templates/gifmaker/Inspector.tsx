...
const confDefault = {
      ...
      const transcode = async () => {
        try {
            ...
            console.log(params)
              ...
              await ffmpeg.writeFile('arial.ttf', await fetchFile('https://ipfs.io/ipfs/QmcWFzXah4SAbKruiDjUN9rgSzztDwJU4F9d6qreRZNF1C'))// font 
              //ffmpeg.exec(["-i",`input.${ty}`,"-ss",`"${params.start}"`,"-t",`"${params.duration}"`,"-r","10","-vf",`"scale=-1:250,drawtext=fontfile=arial.ttf:text='${params.caption}':fontcolor=${params.fontcolor}:fontsize=${params.start}:x=${params.x}:y=${params.y}"`,"output.gif"]); //:x=(w-text_w)/2:y=(h-text_h)/2
              ffmpeg.exec(["-i",`input.${ty}`,"-ss",params.start,"-t",params.duration,"-r","10","-vf",`scale=-1:250,drawtext=fontfile=arial.ttf:text='${params.caption}':fontcolor=${params.fontcolor}:fontsize=${params.fontsize}:x=(w-text_w)/2:y=(h-text_h)-${params.y}`,"output.gif"]); //:x=(w-text_w)/2:y=(h-text_h)/2
   ...
   const create = async () => {
        try { 
        if (!data) {logs.current.value = 'Preview not found. Hit "Privew" button.'; return}
        //updateConfig({ gif: confDefault.gif });
        logs.current.value = 'Creating Frame . . .'

        const b64 = Buffer.from(data).toString('base64');
        const { filePath } = await uploadImage({base64String: b64,contentType: 'image/gif'})
    ...
    return (
        ...
        <div className="w-full h-full space-y-4">
            ...
            <div className="flex flex-col gap-2 ">
                ...
                <Input
                    className="text-lg"
                    placeholder="Position Y (default: 10)"
                    ref={inputY}
                />
                <Input
                    className="text-lg"
...
<img ref={imageRef} width="100%" ></img>
      ...
      >
        Preview
                ...
                <p>
                Note: The recommended gif's duration is less 10 sec.
            </p>
            ...
            <Button
                ...
                onClick={() => {
                    ...
                    inputFontColor.current.value = config.params?.fontcolor || confDefault.fontcolor
